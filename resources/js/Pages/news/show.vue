    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex">
                ...
                <RouterButton :href="route('news.edit', { news: newsOnce.id })"
                    class="inline-flex items-center px-3 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150 cursor-pointer">
        ...
        <div class="py-12 flex justify-center w-full">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="relative flex items-center justify-center mb-4">
                    <img :src="
                        props.newsOnce.base64image
                            ? props.newsOnce.base64image
                            : '/news.jpeg'
                    " alt="News Image" class="object-cover rounded-lg p-2" />
                    ...
                    <div class="absolute bottom-0 left-0 right-0 flex justify-between">
                        <div
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2 text-gray-400"
                                viewBox="0 0 488.85 488.85" fill="currentColor">
                                <path
                        ...
                        <button
                            class="reset-hover inline-flex items-center p-2 rounded-lg bg-white shadow-lg border border-transparent font-semibold text-xs
                             hover:text-white focus:text-white tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150"
                ...
                <div class="bg-white overflow-hidden shadow-lg sm:rounded-lg p-6">
                    <div v-if="props.newsOnce.content" class="text-gray-900 text-lg mt-4">
                        {{ props.newsOnce.content }}
                    </div>
                    <div class="grid grid-cols-2 text-gray-600 text-sm mt-4">
                        <div v-if="props.newsOnce.user?.name" class="flex items-center">
                            {{ props.newsOnce.user.name }}
                        </div>
                        <div v-if="props.newsOnce.published_at" class="flex items-center col-start-2 justify-end">
                            {{
                                new Date(
